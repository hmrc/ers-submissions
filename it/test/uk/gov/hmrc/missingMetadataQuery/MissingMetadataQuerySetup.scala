...
class MissingMetadataQuerySetup(applicationConfig: ApplicationConfig)(implicit ec: ExecutionContext)
  ...
  def schemeInfo(taxYear: String, schemeRef: String, timestamp: Instant): SchemeInfo = SchemeInfo(
    schemeRef = schemeRef,
    timestamp = timestamp,
  ...
  def ersMetaData(taxYear: String, schemeRef: String, timestamp: Instant): ErsMetaData = ErsMetaData(
  ...
  def createMetadataRecord(taxYear: String, schemeRef: String = "XA1100000000000", timestamp: Instant): ErsSummary =
    ErsSummary(
