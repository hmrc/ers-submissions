...
class ResubJobNoFiltersEnabledSpec
  ...
  "ResubPresubmissionServiceJob" should {
    "resubmit failed jobs in batches with the correct transfer status and schema type" in new ResubmissionJobSetUp(app =
      app
    ) {
      ...
      val presubmissionDocsSecondStore: Boolean = await(
        storeMultiplePresubmissionData(
          Fixtures.generatePresubmissionRecordsForMetadata(ersSummariesSecondStore).map(Json.toJsObject(_))
        )
      )
