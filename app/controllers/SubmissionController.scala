      ...
      metadataService.validateErsSummaryFromJson(request.body) match {
        case JsSuccess(ersSummary, _) =>
          (for {
            ...
            case Right(true) =>
              logInfo(s"Submission is successfully completed for: ${ersSummary.metaData.schemeInfo.basicLogMessage}")
              Ok
            case Right(false) =>
              auditEvents.sendToAdrEvent("ErsTransferToAdrFailed", ersSummary)
              logError(s"Processing data failed for: ${ersSummary.metaData.schemeInfo.basicLogMessage}")
              InternalServerError("Processing data failed.")
            case Left(error) =>
              auditEvents.sendToAdrEvent("ErsTransferToAdrFailed", ersSummary)
              logError(s"Processing data failed for: ${ersSummary.metaData.schemeInfo.basicLogMessage} with error: [$error]")
              InternalServerError("Processing data failed.")
          }
      ...
      case JsSuccess(ersSummary, _) =>
        ...
        metadataService.storeErsSummary(ersSummary).value.map {
          case Right(true) =>
            ...
            logInfo(s"ErsSummary is successfully saved, bundleRef: ${ersSummary.bundleRef}")
            Ok("Metadata is successfully stored.")
          case Right(false) =>
            logError(s"Saving ErsSummary failed, bundleRef: ${ersSummary.bundleRef}, ${ersSummary.metaData.schemeInfo.basicLogMessage}")
            auditEvents.auditADRTransferFailure(ersSummary.metaData.schemeInfo, Map.empty)
            InternalServerError("Storing metadata failed.")
          case Left(error) =>
            logError(s"Saving ErsSummary failed, bundleRef: ${ersSummary.bundleRef}, ${ersSummary.metaData.schemeInfo.basicLogMessage} with error: [$error]")
            auditEvents.auditADRTransferFailure(ersSummary.metaData.schemeInfo, Map.empty)
            InternalServerError("Storing metadata failed.")
        }
      case JsError(jsonErrors) => handleBadRequest(jsonErrors)
