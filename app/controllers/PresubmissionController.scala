...
class PresubmissionController @Inject() (
    ...
    request.body.validate[SchemeInfo] match {
      case JsSuccess(schemeInfo, _) =>
        ...
        presubmissionService.removeJson(schemeInfo).value.map {
          case Right(true)    =>
            logInfo(
              s"[PresubmissionController][removePresubmissionJson] Old presubmission data is successfully deleted for: ${schemeInfo.basicLogMessage}"
            )
          ...
          case Left(NoData()) =>
            logWarn(
              s"[PresubmissionController][removePresubmissionJson] No data to delete found for: ${schemeInfo.basicLogMessage}"
            )
            Ok("No data to delete found.")
          case Right(false)   =>
            logError(
              s"[PresubmissionController][removePresubmissionJson] Deleting old presubmission data failed. for: ${schemeInfo.basicLogMessage}"
            )
          ...
          case Left(error)    =>
            logError(
              s"[PresubmissionController][removePresubmissionJson] Deleting old presubmission data failed for: ${schemeInfo.basicLogMessage} with [$error]"
            )
            metrics.failedRemovePresubmission()
            auditEvents.auditADRTransferFailure(schemeInfo, Map.empty)
            InternalServerError("Deleting old presubmission data failed.")
        ...
        case JsSuccess(schemeInfo, _) =>
          ...
          presubmissionService.compareSheetsNumber(validatedSheets, schemeInfo).value.map {
            case Right((true, _))                   =>
              ...
              logInfo(
                s"[PresubmissionController][checkForExistingPresubmission] All presubmission records are found for: ${schemeInfo.basicLogMessage}"
              )
              Ok("All presubmission records are found.")
            case Right((false, sheetsInRepository)) =>
              logError(
                s"[PresubmissionController][checkForExistingPresubmission] Found $sheetsInRepository presubmission records of expected $validatedSheets records for: ${schemeInfo.basicLogMessage}"
              )
              auditEvents.auditADRTransferFailure(schemeInfo, Map.empty)
              InternalServerError(
                s"[PresubmissionController][checkForExistingPresubmission] Not all $validatedSheets records are found for ${schemeInfo.toString}."
              )
            case Left(error)                        =>
              logError(
                s"[PresubmissionController][checkForExistingPresubmission] Check existing presubmission failed for: ${schemeInfo.basicLogMessage} with error: [$error]"
              )
              auditEvents.auditADRTransferFailure(schemeInfo, Map.empty)
              InternalServerError(
                s"[PresubmissionController][checkForExistingPresubmission] Not all $validatedSheets records are found for ${schemeInfo.toString}."
