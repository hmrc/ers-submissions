...
class ADRSubmission @Inject() (
  ...
  def generateSubmission(
    ...
    logInfo(
      s"[ADRSubmission][generateSubmission] ${ersSummary.basicLogMessage} ${ersSummary.metaData.schemeInfo.basicLogMessage}"
    )
  ...
  def createSubmissionJson(ersSummary: ErsSummary, schemeType: String)(implicit
    request: Request[_],
    hc: HeaderCarrier
  ...
  def createSheetsJson(sheetsJson: JsObject, ersSummary: ErsSummary, schemeType: String)(implicit
    request: Request[_],
    hc: HeaderCarrier
  ): ERSEnvelope[JsObject] =
    presubmissionService.getJson(ersSummary.metaData.schemeInfo).map { schemeDataSeq =>
      ...
      if (schemeDataSeq.nonEmpty && sheetNamesAndDataPresent) {
        logInfo(
          s"[ADRSubmission][createSheetsJson] Found data in pre-submission repository, mapped successfully. File data list size: ${schemeDataSeq.size}, ${ersSummary.metaData.schemeInfo.basicLogMessage}"
        )
      } else {
        logWarn(
          s"[ADRSubmission][createSheetsJson] No data returned from pre-submission repository or data is incomplete: ${ersSummary.metaData.schemeInfo.basicLogMessage}"
        )
      }
      ...
      schemeDataSeq.foldLeft(sheetsJson) { (result, fileData) =>
        ...
        val data: JsObject     = buildJson(
          configData,
          fileData.data.get,
          None,
          Some(sheetName),
          Some(fileData.schemeInfo)
        ) // todo: handle fileData.data.get

        result ++ submissionCommon.mergeSheetData(configData.getConfig("data_location"), result, data)
      }
    }

  def createRootJson(sheetsJson: JsObject, ersSummary: ErsSummary, schemeType: String)(implicit
    request: Request[_],
    hc: HeaderCarrier
  ): ERSEnvelope[JsObject] = {
  ...
  def buildJson(
    configData: Config,
    fileData: ListBuffer[scala.Seq[String]],
    row: Option[Int] = None,
    sheetName: Option[String] = None,
    schemeInfo: Option[SchemeInfo] = None
      ...
      elem.getString("type") match {
        ...
        case "array"  =>
          if (fileData.nonEmpty) {
            if (row.isEmpty) {
              val elemVal: List[JsObject] =
                fileData.indices.toList.map(row => buildJson(elem, fileData, Some(row), sheetName, schemeInfo))
