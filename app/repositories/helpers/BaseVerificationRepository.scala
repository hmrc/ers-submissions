...
trait BaseVerificationRepository {
  ...
  lazy val schemeSelector: ERSQuery => BsonDocument = ersQuery => BsonDocument(
    ersQuery.schemeType.map(scheme => s"${mongoKeyPrefix}schemeInfo.schemeType" -> BsonString(scheme)
    )
  )

  lazy val schemeRefsSelector: ERSQuery => BsonDocument = ersQuery => BsonDocument(
    ersQuery.schemeRefsList match {
      case Nil => None
      case nonEmptyList => Some(s"${mongoKeyPrefix}schemeInfo.schemeRef" -> BsonDocument("$in" -> nonEmptyList))
    }
  )
