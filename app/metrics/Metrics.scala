...
class Metrics @Inject() () {
  ...
  def failedStorePresubmission(): Unit =
    SharedMetricRegistries.getOrCreate("failed-store-presubmission").counter("failed-store-presubmission").inc()
  ...
  def failedRemovePresubmission(): Unit =
    SharedMetricRegistries.getOrCreate("failed-remove-presubmission").counter("failed-remove-presubmission").inc()

  def generateJson(diff: Long, unit: TimeUnit): Unit =
    SharedMetricRegistries.getOrCreate("generate-json").timer("generate-json").update(diff, unit)

  def sendToADR(diff: Long, unit: TimeUnit): Unit =
    SharedMetricRegistries.getOrCreate("send-to-ADR").timer("send-to-ADR").update(diff, unit)

  def successfulSendToADR(): Unit =
    SharedMetricRegistries.getOrCreate("successful-send-to-ADR").counter("successful-send-to-ADR").inc()

  def failedSendToADR(): Unit =
    SharedMetricRegistries.getOrCreate("failed-send-to-ADR").counter("failed-send-to-ADR").inc()

  def updatePostsubmissionStatus(diff: Long, unit: TimeUnit): Unit =
    SharedMetricRegistries
      .getOrCreate("update-postsubmission-status")
      .timer("update-postsubmission-status")
      .update(diff, unit)

  def saveMetadata(diff: Long, unit: TimeUnit): Unit =
    SharedMetricRegistries.getOrCreate("save-metadata").timer("save-metadata").update(diff, unit)
