...
class SubmissionService @Inject() (
  ...
  def transformData(ersSummary: ErsSummary)(implicit request: Request[_], hc: HeaderCarrier): ERSEnvelope[JsObject] = {
    ...
    def trimDataIfSizeExceeded(json: JsObject, fieldName: String, jsPath: JsPath, maxLen: Int) = json.transform(
      jsPath.json.update(__.read[JsString].map { field =>
        if (field.as[String].length > maxLen) {
          logInfo(
            s"[SubmissionService][transformData] $fieldName was greater than $maxLen characters for " +
      ...
      case JsError(_)          =>
        logError(
          s"[SubmissionService][transformData] Failed to transform Json Path $jsPath data, attempting to proceed untransformed"
        )
        json
    }
    ...
    adrSubmission.generateSubmission(ersSummary)(request, hc).map { json =>
      ...
      val transformedFirstNameJson =
        trimDataIfSizeExceeded(json, "firstName", __ \ "submitter" \ "firstName", maxFirstNameLen)
      val transformedDataJson      = trimDataIfSizeExceeded(
        transformedFirstNameJson,
        "country",
        __ \ "submitter" \ "address" \ "country",
        maxCountryLen
      )
      transformedDataJson
    }
  ...
  def updatePostsubmission(adrSubmissionStatus: Int, transferStatus: String, schemeInfo: SchemeInfo)(implicit
    hc: HeaderCarrier
  ): ERSEnvelope[Boolean] = {
    ...
    metadataRepository.updateStatus(schemeInfo, transferStatus, Session.id(hc)).flatMap {
      case true if adrSubmissionStatus == ACCEPTED =>
        ...
        logInfo(
          s"[SubmissionService][updatePostsubmission] Updated submission transfer status to: [$transferStatus] for ${schemeInfo.basicLogMessage}"
        )
        ERSEnvelope(true)
      case true                                    =>
        metrics.updatePostsubmissionStatus(System.currentTimeMillis() - startUpdateTime, TimeUnit.MILLISECONDS)
      ...
      case _                                       =>
        logInfo(
          s"[SubmissionService][updatePostsubmission] Submission transfer status update to: [$transferStatus] failed for ${schemeInfo.basicLogMessage}"
        )
        ERSEnvelope(SubmissionStatusUpdateError(Some(adrSubmissionStatus), Some(transferStatus)))
    }
  }
