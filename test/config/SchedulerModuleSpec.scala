...
class SchedulerModuleSpec extends AnyWordSpec with Matchers {
  ...
  "SchedulerModule" should {
    ...
    "bind ResubmissionServiceImpl required to run the resubmission service" in {
      boundClasses should contain(classOf[ResubmissionServiceImpl])
    }

    "bind PreSubWithoutMetadataQueryImpl required to run the pre-sub-without-metadata query" in {
